...
$(document).ready(function() {
    $(document).on('click', '.ordered-item > td.total > a:nth-child(4)', function () {
        ...
        $(this).closest('tr').find('.name').find('span').removeClass('activeComent').removeClass('notActiveComent');
        $(this).addClass('notActiveComent');
        $(this).closest('tr').find('.name').find('span').addClass('activeComent');
    });
    $(document).on('click', '.not-save-comment', function () {
        $(this).parent().addClass('notActiveComent').removeClass('activeComent');
        $(this).closest('tr').find('.add-product-comment').removeClass('notActiveComent');
...
$(document).on('click', ".increase_count", function(){
	var theId = $(this).closest('.product').attr('id');
        var shop = $(this).closest('.product').attr('data-shop');
	...
	newItem = (
	'<tr data-category="'+category+'" class="ordered-item" id="cart-'+$(this).closest('.product').attr('id')+'" data-shop="'+shop+'"> '+
	'<td class="quantity"> '+Number(sessionStorage[theId])+
	'<br>'+'<a href="#" class="cart-change cart-add"><span class="glyphicon glyphicon-arrow-up" aria-hidden="true"></span><span class="cart-del-txt"> Добавить</span></a>'  +
	'<br>'+'<a href="#" class="cart-change cart-min"><span class="glyphicon glyphicon-arrow-down" aria-hidden="true"></span><span class="cart-del-txt"> Убрать</span></a>' +
	'</td>' +
	'<td class="image hidden-xs hidden-sm"> <span class="helper"></span>'+ $(this).closest('.product').find('img')[0].outerHTML + '</td>' +
	'<td class="name">'+$(this).closest('.product').find('.product-title').html()+
	'<span><textarea name="text" class="special-instructions-box"></textarea><a class="editComents" href="javascript:void(0)"><i class="fa fa-pencil-square-o" aria-hidden="true">  </i> Редактировать</a><a href="javascript:void(0)" class="ssave-comment">Сохранить</a><a href="javascript:void(0)" class="not-save-comment">Отмена</a></span>'+
	'</td>'+
...
$(document).on('click', '.add-to-cart', function(e){
	...
	'<td class="name">'+$(this).parents(".modal-dialog").find(".popup-name").find('.popup-title').html()+
	'<span><textarea name="text" class="special-instructions-box"></textarea><a class="editComents" href="javascript:void(0)"><i class="fa fa-pencil-square-o" aria-hidden="true">  </i> Редактировать</a><a href="javascript:void(0)" class="ssave-comment">Сохранить</a><a href="javascript:void(0)" class="not-save-comment">Отмена</a></span>'+
	'</td>'+
	...
	search_results = '';
        ...
        $.ajax({
            ...
            success: function(res){
			    ...
			    search_results  += '<div class="col-lg-1 col-md-2 col-sm-3 col-xs-6 product-wrap product" id="' + json[i].objectId + '" data-category="'+json[i].category+'" data-shop="'+json[i].shop+'"><div class="count item_count visible">'+sessionStorage[json[i].objectId]+'</div><a href="#" data-toggle="modal" data-target="#idt' + json[i].objectId + '"> <img src="' + json[i].img_sm + '" alt=""></a><div class="desc"><div class="product-title"> <a href="#" data-toggle="modal" data-target="#idt' + json[i].objectId + '">' + json[i].product_name + '</a></div><div class="col-lg-6 col-md-8 col-xs-12 col-sm-8 price">' + json[i].price + ' руб</div><div class="col-lg-6 col-md-4 col-xs-12 col-sm-4 quantity">'+ json[i].amount +'</div><br class="clearfix"></div><button href="javascript:void(0)" data-weight="'+json[i].weight+'" class="increase_count buy">Добавить</button></div>';

			} else {
			    search_results  += '<div class="col-lg-1 col-md-2 col-sm-3 col-xs-6 product-wrap product" id="' + json[i].objectId + '" data-category="'+json[i].category+'" data-shop="'+json[i].shop+'"><div class="count item_count hidden">'+sessionStorage[json[i].objectId]+'</div><a href="#" data-toggle="modal" data-target="#idt' + json[i].objectId + '"> <img src="' + json[i].img_sm + '" alt=""></a><div class="desc"><div class="product-title"> <a href="#" data-toggle="modal" data-target="#idt' + json[i].objectId + '">' + json[i].product_name + '</a></div><div class="col-lg-6 col-md-8 col-xs-12 col-sm-8 price">' + json[i].price + ' руб</div><div class="col-lg-6 col-md-4 col-xs-12 col-sm-4 quantity">'+ json[i].amount +'</div><br class="clearfix"></div><button href="javascript:void(0)" data-weight="'+json[i].weight+'" class="increase_count buy">Добавить</button></div>';
